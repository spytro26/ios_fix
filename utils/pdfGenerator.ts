...
const getLogoBase64 = async (): Promise<{logo: string, watermark: string, status: string}> => {
  let errorMessage = '';
  ...
  try {
      ...
      if (response.ok) {
        ...
        return new Promise((resolve) => {
          ...
          reader.onloadend = () => {
            ...
            if (result && result.length > 50) { // Basic validation - base64 should be much longer
              resolve({
                logo: result,
              ...
              resolve({
                ...
                status: `Logo.png loaded but data was invalid (length: ${result?.length || 0})`
              });
            }
          };
          reader.onerror = () => {
    ...
    errorMessage = `Fetch method failed: ${fetchError?.message || 'Unknown fetch error'}`;
  }
  ...
  try {
    ...
    errorMessage += ` | File API failed: ${fileError?.message || 'Unknown error'}`;
  }
...
export const generateAndSharePDF = async (data: PDFData) => {
  try {
    ...
    const filename = `${data.title.replace(/[^a-zA-Z0-9]/g, '_')}_${timestamp}.pdf`;
    ...
    if (error instanceof Error) {
      if (error.message?.includes('User cancelled')) {
        ...
        errorMessage = 'Permission denied. Please check your device permissions for sharing files.';
      }
    }
...
const generateHTMLContent = (
  ...
  const normalizeRoomDefinitionInputs = (inputs: NonNullable<PDFData['inputs']>): NonNullable<PDFData['inputs']> => {
    try {
      return inputs.map((section) => {
        const title = (section?.title || '').toLowerCase();
        const isRoomDefinition = title.includes('room') && title.includes('definition');
        if (!isRoomDefinition) return section;

        const items = Array.isArray(section.items) ? [...section.items] : [];
        const findItem = (kw: string) => items.find(it => (it?.label || '').toLowerCase().includes(kw));
        ...
        if (len && wid && hgt) {
          const fmt = (it: any) => `${it?.value ?? ''}${it?.unit ? ' ' + it.unit : ''}`.trim();
          const combined = {
          ...
          const filtered = items.filter(it => it !== len && it !== wid && it !== hgt);
  ...
  const normalizedInputs = data.inputs ? normalizeRoomDefinitionInputs(data.inputs) : null;
  const slugify = (s: string) => (s || '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/(^-|-$)/g, '');
  // Helper to render a section table
  const renderSection = (inputSection: NonNullable<PDFData['inputs']>[number]) => `
        ...
        <table class="data-table">
          ...
          <tbody>
            ${(inputSection.items || []).map(item => `
              <tr>
                <td class="table-label">${item.label ?? ''}</td>
                <td class="table-value">${[item.value, item.unit].filter(Boolean).join(' ')}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
  ...
  if (normalizedInputs) {
    if (isIOS) {
      const findTitle = (kw: string) => normalizedInputs.find(s => (s.title || '').toLowerCase().includes(kw));
      const ambient = findTitle('ambient');
      const product = findTitle('product');
      const room = findTitle('room');
      const internal = findTitle('internal');
      const others = normalizedInputs.filter(s => s !== ambient && s !== product && s !== room && s !== internal);
  ...
  body { -webkit-text-size-adjust: 100%; -webkit-font-smoothing: antialiased; padding-bottom: 8px; }
      ...
      <div class="header">
        <div class="header-left">
          <div class="header-logo">
            ${logoBase64 ? `<img src="${logoBase64}" alt="Logo" />` : '<div style="color: #1e40af; font-weight: bold; font-size: 16px;">E</div>'}
          </div>
